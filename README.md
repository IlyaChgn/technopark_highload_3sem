...
Использование Kubernetes для распределения межсервисных запросов в зависимости от состояния конкретного контейнера.
...
#### ClickHouse
Будем шардировать таблицы Comment и View, так как у них предполагается самый большой объем данных. Для разделения по шардам будем использовать video_id в первом случае и user_id во втором.
...
- При завершении работы сервисов должна выполняться корректная обработка всех активных запросов для предотвращения потери данных.
- Настройка таймаута на основе квантилей для переключения слишклм долгих запросов на менее загруженные сервера.
