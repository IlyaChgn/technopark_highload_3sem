...
- Равномерное распределение входящих запросов между серверами с помошью Round Robin. Это можно использовать для низкой интенсивности запросов, таких как страницы рекомендаций и поиск видео
...
- Для тех же целей можно использовать Weighted Round Robin, но уже применительно к другим запросам: комментарии или лайки
...
- Примерный расчет загрузки: при пиковом RPS на один ДЦ в среднем приходится 514 RPS, Если обмен ключами происходит за 0,2 с, то для обмена потребуется 100 секунд процессорного времени, то есть необоходимо иметь 100 ядер
