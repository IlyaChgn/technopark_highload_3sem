...
- Для снижения нагрузки на БД добавим поля с количеством комментариев и лайков конкретного видео, обновление этих полей будет происходить при обновлении вышеупомянутых таблиц
...
- Максимальная длина электронной почты согласно RFC 5321 сотавляет 320 символов [9]
- Максимальная длина комментария и описания к видео на видеохостинге составляет 5000 символов [10][11]
- Максимальная длина заголовка видео - 100 символов [11]
...
#### User
 ...
 Итого: 670 байт на одного пользователя без учета аватара и шапки канала
 Суммарный объем данных: 670 байт * 2,7 млрд. пользователей = 1,65 Тб
...
Суммарный объем данных: 5100 байт * 4 млрд. видео = 18,5 Тб
...
Суммарный объем данных: 5040 байт * 0,002 * 4 млрд. видео * 5600 = 205 Тб
Здесь 0,002 - процент пользователей, которые оставляют комментарии, 5600 - среднее количество просмотров у видео
...
Суммарный объем данных: 24 байта * 0,004 * 4 млрд. видео * 5600 = 2 Тб
Здесь 0,004 - процент пользователей, которые оставляют лайки, 5600 - среднее количество просмотров у видео
...
| Video        | 5100 байт             | 2824                    | 37 (9 из них - обновление числа лайков и комментариев)                      |
| Comment      | 5040 байт             | 28240                   | 3                       |
...
Для осущетсвления быстрого поиска по заголовку видео будем использовать СУБД ElasticSearch, в которой будет храниться заголовок видео, его автор, дата публикации и UUID в S3-хранилище. Такой подход позволит обеспечить быстрый поиск всех данных о видео, которые необходимо отобразить на странице поиска.

### Денормализация
Денормализация была проведена в прошлом пункте. С этой целью были выделены поля comments_count и likes_count в таблице Video, а также поле subscribers_count в таблице User. Эти поля предназначены для получения дополнительной информации о видео или об авторе, запрос на получение которой из основных таблиц будет более дорогостоящей операцией ввиду их размера, а само получение данных необходимо каждый раз, когда происходит загрузка страницы видео или страницы автора.
...
#### PostgreSQL
Применим шардирование для таблиц Video, Comment и User, так как это три самые тяжелые таблицы, а последние две также являются самыми нагруженными по чтению данных из них. Распределять по шардам будем на основе первичных ключей для Video и User, а также внешнего ключа video_id для таблицы Comments. Используем при этом следующий алгоритм:
- Вычисляем хеш от первичного ключа
- Определяем по нему номера шарда, как остаток деления хеша на число шардов
- Производим операцию по номеру нужного шарда

Будем использовать репликацию по схеме master-slave с использованием трех синхронных slave-узлов. Такая схема поможет сохранить данные в случае отказа master-сервера.
...
Будем использовать репликацию по схеме master-slave с использованием двух асинхронных slave-узлов. Данная схема позволит прочитать данные о сессии пользователя даже в случае отказа одного узлов, что является критически важным для приложения, в котором многое ориентировано на наличие авторизации у пользователя.

#### ElasticSearch
Для настройки шардирования в ElasticSearch разобьем индекс на несколько щардов с помощью внутренних средств СУБД. Это поможет ускорить поиск видео по их заголовку, так как сузить объем данных.
...
Будем писать серверную часть приложения на языке Go, так как этот язык обладает множеством библиотек с поддержкой различных СУБД. В данном случае можно использовать следующие:
