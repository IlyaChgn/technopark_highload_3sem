...
- При расчете физических ресурсов будем исходить из следующего:
    + 100 RPS на 1 CPU
    + 1000 RPS на 1 Гб RAM
- Nginx способен обрабатывать до 38 400 HTTPS-запросов на 8 ядрах [14]. Этого будет достаточно для поддержания работоспособности приложения.


|    Сервис     |    RPS    |    CPU    |    RAM    |
| ------------- | --------- | --------- | --------- |
| API gateway   | 8661      | 86        | 12 Гб     |
| video         | 813       | 9         | 1 Гб      |
| user          | 813       | 9         | 1 Гб      |
| auth          | 3753      | 38        | 4 Гб      |
| search        | 980       | 10        | 1 Гб      |
...
| ratings       | 785       | 8         | 1 Гб      |
| views         | 785       | 8         | 1 Гб      |
| subscriptions | 785       | 8         | 1 Гб      |
| statistics    | 1626      | 17        | 2 Гб      |
| Nginx         | 19560     | 8         | 20 Гб     |

### Модель хостинга
В качестве модели хостинга выберем собственное железо. В связи с тем, что сервис должен выдерживать высокий уровень нагрузки и иметь ДЦ в разных точках Земли, это будет лучшим вариантом.

### Конфигурации серверов
Для развертывания приложения будем использовать Kubernetes. 

Все сервисы, кроме API gateway, Nginx, auth и statistics будут использовать сервера в одинаковой конфигурации. В таблице они обозначены как kubenode. Для них в одном ДЦ требуется суммарно 60 ядер.
| Сервис | Хостинг | Конфигурация | Cores | Count | Покупка | Амортизация |
| ------ | ------- | ------------ | ----- | ----- | ------- | ----------- |
| kubenode | own | 2x6338/16x32GB/1xNVMe1T/2x25Gb/s | 64 | 9 | $14500 | $241 |
| auth, statistics, Nginx | own | 2x6338/16x32GB/1xNVMe1T/2x25Gb/s | 64 | 9 | $14500 | $241 |
...
14. [Провизводительность Nginx](https://blog.nginx.org/blog/testing-the-performance-of-nginx-and-nginx-plus-web-servers)